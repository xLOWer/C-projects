...
void my_func1(){
	double x = 60, y = 60, r = 40, fi = 0, angle = 35;
	while(1){
		Ellipse(hdc,x - r, y - r, x + r, y + r);
		pause(.000009); 
		SQUARE(0,0,W_SIZE_W); 
		if(y - r <= 0 || x - r <= 0 || y + r > 700 || x + r >  700){ angle = 360 - angle + 180; }
		fi = (angle + 90) * M_PI / 180;
		x += sin(fi);y += cos(fi);
		system("cls"); std::cout << "x: " << x << "\ny: " << y  << "\nH: " << W_START_Y + W_SIZE_H << "\nW: " << W_START_X + W_SIZE_W << "\na: " << angle;
	}
}
//***********************************************************************
...
/*for(i = 0; i <= 36000; i += angle){fi = i * M_PI / 180;R = 5 / (3 * (cos(fi) + 2));x = r * R * cos(fi);y = r * R * sin(fi);PIXEL(hdc, x+dx, y+dy);}*/	
			//DRAWW(pos, 1, 5);	
			my_func1();		
			EndPaint(hwnd, &pt);
			break;
